...
export class DashboarddetailComponent implements OnInit,AfterViewInit {
  ...
  public roleName:any;
  Name:string;
  filteredItems: { moduleName: string;moduleGroup:string;totalCount: number; lineItem1Text: string; lineItem1Count: number; lineItem2Text: string; lineItem2Count: number;icon: string }[] = [];
  counterMap: { [moduleName: string]: number } = {};
  filteredExperts: { moduleName: string;moduleGroup:string; totalCount: number; lineItem1Text: string; lineItem1Count: number; lineItem2Text: string; lineItem2Count: number;icon: string }[] = [];
  ...
  constructor(
    ...
    private routes: ActivatedRoute, private authservice: AuthService,
     private menuService: MenuService, private univservice: UniversityidService, private encoderservice: NumencoderService
  ) {
  ...
  ngOnInit() {
    ...
    this.routes.params.subscribe(param => {
      this.univId = this.encoderservice.decodeNumber(param.data);
    });
    ...
    this.service.GetDashboard(this.userDetail.UniversityId,this.userName).subscribe(
      (x) => {
      this.models = x;
      ...
      item.moduleName!="Faculty"&&item.moduleName!="Scholar"&&item.moduleName!="Startups")
