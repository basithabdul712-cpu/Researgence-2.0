...
import { Facultylist } from '../faculties-list/facultylist';
...
export class FacultiesListComponent implements OnInit {
    ...
    constructor(public service:FacultiesService, private router:Router, private menuService: MenuService,private univservice: UniversityidService,
        private search:CommonsearchService,private authservice: AuthService,private http:HttpClient, private encoderService: NumencoderService){  
      } 
   
    ngOnInit() {  

        this.uibaseurl='/facultyProfiles'; 
        this.compareurl='/facultyProfiles/compare/fac';
      ...
      //For rolename getting
          // this.authservice.RoleSelection(this.userDetail.UniversityId,'').subscribe(x=>{
          //   this.role=x;
          ...
          //   })       
           ...
           this.service.getUnivLocSchInstDept(this.univId,this.userRole,'',this.locationFilterId,this.schoolFilterId,this.instituteFilterId,this.deptFilterId).subscribe(x=>{
            this.layerFilter=x;
           ...
           this.service.getUnivCheckModule(this.univId).subscribe(t=>{  
            ...
            let pubList=this.moduleList.filter(x=>x.moduleName=="Publications"||x.moduleName=="Patents");
               for(let i=0;i<pubList.length;i++){
                 if(pubList[i].moduleName=="Publications"){
                      this.enablePub=pubList[i].isApplicable;
                 }
                 else if(pubList[i].moduleName=="Patents"){
                       this.enablePat=pubList[i].isApplicable;
                 }
               }
              }
          });
          ...
          onWindowScroll() {
              ...
              fetchData() {
                ...
                this.data.universityId = this.univId;
                   this.userRole=6;
                ...
                this.service.getfacultyListPage(this.data)
                    ...
                    for (let i = 0; i < this.filteredItems.length; i++) {
                      ...
                      if (Array.isArray(this.filteredItems) && this.filteredItems[i].hasOwnProperty('userProfilePicLocation')) {
                          ...
                          if (this.imageToShow.find(x => x == this.filteredItems[i].empId)) {
                            ...
                            this.filteredItems[i].userProfilePicLocation="/"+this.filteredItems[i].userProfilePicLocation;
                            const userProfilePicLocation: string[] = this.filteredItems[i].userProfilePicLocation;
                              const imageUrl = `${environment.nodeServerUrl}/getImage?userImage=${userProfilePicLocation}`;
                              let imageUrlWithoutExtension = imageUrl;
                              // console.log(imageUrlWithoutExtension);
                              this.http.get(imageUrlWithoutExtension, { responseType: 'text' }).subscribe(
                                  (response) => {
                                      this.filteredItems[i].userProfilePicLocation = imageUrlWithoutExtension; // Update the imgUrl with the fetched image URL
                      ...
                      if (this.imageToShow.find(x => x == this.filteredItems[i].empId)) {
                        ...
                        const userProfilePicLocation: string[] = this.filteredItems[i].userProfilePicLocation;
                          ...
                          this.http.get(imageUrlWithoutExtension, { responseType: 'text' }).subscribe(
                              ...
                              (error) => {
                                  // console.error('Error fetching Image:', error);
                                  imageUrlWithoutExtension = "assets/images/user/default.png";
                              });
                      }
                  } else {
                    this.filteredItems[i].userProfilePicLocation ="assets/images/user/default.png"; 
          ...
          if(val=="loc"){
            ...
            if(val=="scl"){
              ...
              if(this.filterSchool==""){
                ...
                if(this.layerType=="4LType2"||this.layerType=="3LType3"){
                  ...
                  const schoolfilter=this.layerInsSchCamDep.filter(item=> item.schoolName==this.filterSchool&&item.locationName==this.filterCamp);
                  this.schoolFilterId=schoolfilter[0].schoolId;
                }       
              }
            ...
            if(val=="inst"){
                  ...
                  if(this.layerType=="3LType1"){
                    const instfilter=this.layerInsSchCamDep.filter(item=>item.instituteName==this.filterInst&&item.locationName==this.filterCamp)
                    this.instituteFilterId=instfilter[0].instituteId;
                  }
                  else if(this.layerType=="4LType2"||this.layerType=="3LType3"){
                    const instfilter=this.layerInsSchCamDep.filter(item=>item.instituteName==this.filterInst&&item.schoolName==this.filterSchool)
                    this.instituteFilterId=instfilter[0].instituteId;
                  }
                  else{
