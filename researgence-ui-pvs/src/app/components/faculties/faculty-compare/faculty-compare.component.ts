...
import {FacultyCompareData} from '../faculty-compare/facultycomparedata';


@Component({
  selector: 'app-faculty-compare',
  templateUrl: './faculty-compare.component.html',
  styleUrls:['./faculty-compare.component.scss','./../../../../assets/given/newcss/style.css', './../../../../assets/given/selected.css']

})
export class FacultyCompareComponent implements OnInit {
  ...
  ngOnInit() {  
    ...
    if(this.userRole==12){
      var tempUnivId=localStorage.getItem("NewUnivId");
      this.service.getLayerType(tempUnivId,this.userRole,this.user.UserId).subscribe(x=>{
        console.log(x);      
        this.layerList=x;  
    ...
    // this.Name = this.user['http://schemas.xmlsoap.org/ws/2005/05/identity/claims/name'];
      ...
      // this.authservice.RoleSelection(this.univId, this.user.UserId).subscribe(x => {
      //   this.role = x;
      //   const data = this.role.filter(item => item.roleId == this.userRole);
      //   this.roleName = data[0].roleName;
      //   console.log(this.roleName)
      // })
    this.service.getUnivLocSchInstDept(this.user.UniversityId,this.userRole,'',this.locationFilterId,this.schoolFilterId,this.instituteFilterId,this.deptFilterId).subscribe(x=>{
      this.layerFilter=x;
      console.log(x);  
    ...
    getCompareExcelData() {
      ...
      if(this.roleId==12){  
        this.facultyCompareData.universityId = parseInt(localStorage.getItem("NewUnivId"));
      }
      else{
        this.facultyCompareData.universityId = this.univId;
      }
     
      if(this.roleId==12){
        this.facultyCompareData.roleId = 6;
      }
      else{
      ...
      this.service.ResearcherCompare(this.facultyCompareData)
        .subscribe(x => {
          this.excelList = x;
      ...
      getFacultyCompareDetails() {
        ...
        this.service.ResearcherCompare(this.facultyCompareData).subscribe(
          data => {
            console.log(data)
    ...
    onOpenCalendar(container) {
      ...
      container.setViewMode('month');
        ...
        if(val=="loc"){
          ...
          this.service.getUnivLocSchInstDept(this.univId,this.userRole,this.user.UserId,this.locationFilterId,this.schoolFilterId,this.instituteFilterId,this.deptFilterId).subscribe(x=>{
            console.log(x);
          ...
          if(val=="dept"){
            ...
            if(this.filterDepart==""&&this.filterInst==""&&this.filterCamp==""&&this.filterSchool==""){
              ...
              if(this.roleId==12){
                ...
                if(this.startrow==0){
                  this.facultyCompareData.endRow = this.pageSize;
                }
                else{
                  this.facultyCompareData.endRow = this.endrow;
                }
