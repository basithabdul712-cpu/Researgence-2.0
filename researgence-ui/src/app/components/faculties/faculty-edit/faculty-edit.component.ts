        ...
        ngOnInit() {
          ...
          if(this.roleId=='12'||this.roleId=='5'||this.roleId=='17'){
            ...
            this.service.SearchUniversity().subscribe(x=>{
                  ...
                  if(this.roleId=='5'||this.roleId=='17'){
          ...
          this.service.getUnivLocSchInstDept(this.universid,this.roleId,this.user.UserId,null,null,null,null).subscribe(x=>{
                ...
                this.service.getProfileEdit(this.universid,this.userid).subscribe(x=>{
                  ...
                  if(this.roleId=='12'){
                    ...
                    this.service.SearchUniversity().subscribe(x=>{
                      this.tempUnivList=x;
                      let filterList=this.tempUnivList.filter(item=>item.id==this.existDetails.universityId);
                      this.universityName=filterList[0].name
                    });                    
                  }
                ...
                if(this.filteredDesignations&&this.cidDropdown){
                 ...
                 if(this.roleId=='5'||this.roleId=='12'){   
                  ...
                  this.service.getRoleList(this.universid,this.roleId).subscribe(x=>{
                    ...
                    this.postDoc=this.tempRoleId.includes('18');
                   }
                 }
                  else{
                    this.universityId=this.existDetails.universityId;
                ...
                if(this.existDetails.locationId!=null){
                  this.layerCampus=Array.from(new Set(this.tempList.filter(item => item != null && item.locationName != null).map(item => item.locationName)));
                  const filterData = (data: any[]) => {
                    return data.filter(item => item.locationName !== null);
                  };
                  this.layerCampus=filterData(this.layerCampus);
                  if(this.layerType=="3LType1"||this.layerType=="2LType1"){
                    this.layerSchool=Array.from(new Set(this.tempList.filter(item => item != null && item.schoolName != null).map(item => item.schoolName)));
                    this.layerInst=Array.from(new Set(this.tempList.filter(item => item != null && item.instituteName != null).map(item => item.instituteName)));
                    const filterData = (data: any[]) => {
                    ...
                    console.log(this.layerInst);
                        ...
                        this.filteredDept=filterData(this.filteredDept);
        ...
        onFileSelected(event: any) {
          ...
          if (this.file) {
            ...
            if(this.file.type=="image/jpeg"||this.file.type=="image/png"||this.file.type=="image/jpg"){
              ...
              alert("Please select formatted image");
              this.file = null;
              event.target.value = '';
            }
          }
...
UpdateData(){
   ...
   if(this.file){
    this.fileName=this.imagePathData.folderPath+"\\"+this.userid;
  ...
  //   if (this.universityName == this.universityFilter[0]?.name) {
      ...
      if (this.filteredUniversity && this.filteredUniversity.length > 0) {
        ...
        console.error("this.filteredUniversity is not defined or empty.");
          ...
          if(this.genderName==""){
            ...
            else if(this.genderName==this.tempGenName){
              this.genderId = this.existDetails.genderId;
            }
            else{
      ...
      const Data={
          ...
          if(this.fullName!=""){
            if(!this.file){          
            this.service.updateProfile(Data,this.userId,this.roleId,this.user.UserId).subscribe(x=>{                         
             const confirmation = confirm("Details updated successfully");
              if (confirmation) {
                if(this.roleId=="1"||this.roleId=="2"||this.roleId=="6"||this.roleId=="7"||this.roleId=="8"||this.roleId=="3"||this.roleId=="9"||this.roleId=='10'||this.roleId=='16'||this.roleId=='18'){
        ...
        fetchImage() {
          ...
          this.service.GetPath(this.universid,this.userid,'1').subscribe(x=>{
            this.imgPath=x;
            ...
            if(this.imgPath){  
              if(this.imgPath.proposedFileName!=null){
                ...
                this.userImage=this.imgPath.folderPath+"\\"+this.userid;
              }
      ...
      updateValues(val) {
        ...
        if (this.postDoc && !this.temproles.includes('18')) {
          dataParts.push('18');
        }
        ...
        if(this.temproles==""){
          ...
          else if (!this.postDoc&&val=='18') {
            this.temproles = this.temproles.split(',').filter(roleId => roleId !== '18').join(',');
          }
      ...
      changePassword(modal: any){
        ...
        this.authservice.getUserMail(this.userid).subscribe(x=>{
          this.mailValue=x;
          console.log(this.mailValue);
          
          this.mail=this.mailValue.value;
      ...
      processData(){
         if(this.currentPassword!=this.userCredential){
            alert("Please check current password");
             ...
             if(this.newPassword!=undefined||this.cnfPassword!=this.cnfPassword){
                if(this.newPassword==this.cnfPassword){
                    this.mismatchMsg=false;
                        const confirmation = confirm('Please confirm to change password');           
                        if (confirmation) {
                          this.authservice.updateMail(this.userid,this.cnfPassword,this.mail,"1").subscribe(x=>{
