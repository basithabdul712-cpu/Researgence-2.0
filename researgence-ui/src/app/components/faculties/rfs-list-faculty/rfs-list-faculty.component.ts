...
gotoedit(requestId){
  console.log(this.isReadOnly);
  this.router.navigate(['/facultyProfiles/faculty/rfs-edit',requestId,'publication']);
}
...
const confirmation = confirm('Confirm to Delete');
   ...
   if(confirmation){
this.facultyservice.DeleteRfs(this.user.UserId,this.userName,data,this.user.UserId).subscribe(x=>{
                    ...
                    if (!this.pdfStatus && this.pdfEnable) {
                      ...
                      if (this.rfsId == "3") {
                        if (this.pdfLocation != null && this.crossTitle != undefined && this.crossSourceName != undefined && this.webLinkAdd != undefined) {
                          this.facultyservice.SaveRFS(this.user.UserId, this.userName, data,this.user.UserId).subscribe(x => {
