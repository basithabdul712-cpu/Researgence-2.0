...
@Component({
  selector: 'app-publication-mine',
  templateUrl: './publication-mine.component.html',
  styleUrls: ['./publication-mine.component.scss','../../../../../assets/given/newcss/style.css','../../../../../assets/given/newcss/splide.min.css','../../../../../assets/given/selected.css', '../../../../../assets/given/css/style-vit1.css', '../../../../../assets/given/css/style-vit2.css','../../../../../assets/given/newcss/bootstrap.min.css']
})
export class PublicationMineComponent implements OnInit {
...
@ViewChild('pdfTable', { static: false }) pdfTable: ElementRef;
  ...
  constructor( private router:Router,private route: ActivatedRoute,private authService:AuthService,private scoreservice:ScorebookService,private modalService: NgbModal,
  ...
  ngOnInit(){ 
    ...
    //  });
      ...
      this.route.params.subscribe(params => {    
        ...
        this.type=params['View'];

           if(this.type=="EditedPublication"){
            this.pubSearchList=[  {
      ...
      this.selectedTab = 'apa';
          ...
          if(checkString.roleId=="11"||checkString.roleId=="12"){
            checkString.roleId=6;
            ...
            // Assigning variable for showing values dynamically in honeycomp
              for(let t=0;t<this.dataList[i].publicationDBCitation.length;t++){
                ...
                this.dataList[i].authorAffiliation = this.dataList[i].authorAffiliation.split('|');
                 ...
                 if(this.dataList[i].editorAffiliation!=null&&this.dataList[i].isEditedPublication=="True"){
                  this.dataList[i].editorAffiliation = this.dataList[i].editorAffiliation.split('|');  
                }
              }
            ...
            this.scoreservice.fetchMIneDB(checkString).subscribe((x: HttpResponse<any>)=>{ 
              if (x.status === 200) {
                this.mineData = x.body;
                this.enablePic = true;
              } else {
...
onPageSizeChange(size: string){
 ...
 setExcel():Observable<any>{
  ...
  "isInHouse","authorVerifiedStatus","authorAffiliationHTML","publicationDBCitation","publicationSourceDBMetrics","homeFaculty","homeScholar","homeStaff","correspondingAuthor", "isNIRFEligible", "isNACCEligible", "isQSEligible", "isTHEEligible", "isIndexed","createdDate"
  ];

  this.getList.endRow=this.collapsablesize;
      return  this.scoreservice.fetchMIneDB1(this.getList).pipe(
        tap((x: any) => {      
          this.excelData=x;     
        ...
        this.tempList.forEach(publication => { 
          ...
          if(typeof publication.ePage==='string'&&publication.ePage!=null){
            publication.ePage = Number(publication.ePage);
              ...
              if (typeof publication.scs === 'string') {
                  if (publication.scs.trim() === '') {
                      publication.scs = null;
                  } else {
                      publication.scs = Number(publication.scs);
                  }
              }
              if (typeof publication.wos === 'string') {
                  if (publication.wos.trim() === '') {
                       publication.wos = null;
                  } else {
                      publication.wos = Number(publication.wos);
                  }
               }
               if (typeof publication.gs === 'string') {
                    if (publication.gs.trim() === '') {
                        publication.gs = null;
                    } else {
                        publication.gs = Number(publication.gs);
                    }
               }
               if (typeof publication.ieee === 'string') {
                    if (publication.ieee.trim() === '') {
                        publication.ieee = null;
                    } else {
                        publication.ieee = Number(publication.ieee);
                    }
               }
               if (typeof publication.pm === 'string') {
                  if (publication.pm.trim() === '') {
                      publication.pm = null;
                  } else {
                      publication.pm = publication.pm;
                  }
                }
 ...
 exportexcel(){
  ...
  str = str.replace(/\"homeFaculty\":/g, "\"HOME FACULTY\":");
  str = str.replace(/\"homeScholar\":/g, "\"HOME SCHOLAR\":");
  str = str.replace(/\"homeStaff\":/g, "\"HOME STAFF\":");
  str = str.replace(/\"correspondingAuthor\":/g, "\"CORRESPONDING AUTHOR\":");
  str = str.replace(/\"isNACCEligible\":/g, "\"NAAC\":");
  str = str.replace(/\"isNIRFEligible\":/g, "\"NIRF\":");
  str = str.replace(/\"isQSEligible\":/g, "\"QS\":");
  str = str.replace(/\"isTHEEligible\":/g, "\"THE\":");
  str = str.replace(/\"isIndexed\":/g, "\"IsIndexed\":");
  str = str.replace(/\"createdDate\":/g, "\"CREATED DATE\":");
  ...
  this.tempList.forEach((x) => {
    ...
    delete x.homeUniversity; delete x.crossRf;
  });
  ...
  exportToExcel() {
    ...
    if(this.roleId==12||this.roleId==11){
      this.setExcel1().subscribe(() => {
      });
    }
    else{
  ...
  setExcel1():Observable<any>{
    ...
    console.log(this.tempList);
     ...
     exportexcel1(){
      ...
      this.excel.exportPubMineExcelSupport(this.tempList, "Support_Pub_Mine");
    }
