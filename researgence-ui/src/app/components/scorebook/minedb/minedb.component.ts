...
@ViewChild('blueheader') blueheader!: ElementRef<HTMLElement>;
  ...
  levelId:any;
  doi:string;
  databaseDropDownRanking: any;
  rankingValue: any;
  rankingId: any;
  selectAllRankingPubType: boolean=false;
  ...
  constructor( private router:Router,private authService:AuthService,private scoreservice:ScorebookService,private modalService: NgbModal,
    private menuService:MenuService,private facultyservice: FacultiesService,private fb: FormBuilder,private service:AdminclientService) {
      this.checkedList = [];
      this.checkedListJournal=[];
      this.selectDBJournal=[];
      this.selectDB=[];
  ...
  ngOnInit() {
    this.insight='/scorebook/Publications/InsightReports';
      ...
      if(this.roleId==11||this.roleId==12){
        ...
        this.facultyservice.getUncategoryUnivLocSchInstDept (this.user.UniversityId,this.roleId,this.user.UserId,this.locationFilterId,this.schoolFilterId,this.instituteFilterId,this.deptFilterId).subscribe(x =>{
          this.campus =x;
          console.log(x);
      ...
      this.facultyservice.getDropdown('publicationSourcedb').subscribe(x=>{
        this.databaseDropDownJournal=x;
      })

      //For Ranking 
      this.facultyservice.getDropdown('Ranking').subscribe(x=>{
        this.databaseDropDownRanking=x;
        if(this.databaseDropDownRanking.length>0){
          for(let i=0;i<this.databaseDropDownRanking.length;i++){
            this.databaseDropDownRanking[i].isSelected=false;
          }
        }  
      })
      ...
      if (storedTitle !== null && storedTitle !== undefined) {
        ...
        for(let i=0;i<checkString.searchList.length;i++){
           if(checkString.searchList[i].columnName==="AuthorName"){
            ...
            //Changed for ranking
          // if(checkString.searchList[i].columnName==="RANKING"){
          //   this.rankingValue=checkString.searchList[i].searchValue;
          //   this.rankingId=checkString.searchList[i].searchId;
          //  }
           ...
           if(checkString.searchList[i].columnName==="Level"){
            this.level=checkString.searchList[i].searchValue;
            this.levelId=checkString.searchList[i].searchId;
           }
        }
    ...
    selectAllRanking() {
      this.selectAllRankingPubType = !this.selectAllRankingPubType;
      this.databaseDropDownRanking.forEach(item => item.isSelected = this.selectAllRankingPubType);
    ...
    toggleCheckboxRanking(item){
      item.isSelected = !item.isSelected;
      this.selectAllRankingPubType = this.databaseDropDownRanking.every(i => i.isSelected);
 
      this.updatePubtypeRanking();
    }
    ...
    updatePubtypeRanking() {
      this.rankingValue = this.databaseDropDownRanking.filter(item => item.isSelected).map(item => `'${item.value}'`).join(',');
      console.log(this.rankingValue);
    }
        ...
        searchData(){
          ...
          this.selectedTab = 'apa';
            ...
            if(this.publicationTitle!=null||this.tempfromYear!=undefined||this.checkedList.length>0||this.temptoYear!=undefined||this.pubtype!=undefined||
              this.authorname!=undefined||this.facultyname!=undefined||this.journalname!=undefined||this.Campusfiter!=undefined||this.filterSchool!=undefined||this.Institutefilter!=undefined||this.departmentfilter!=undefined||this.quartile!=undefined||this.issn!=undefined||this.isbn!=undefined||this.rankingValue!=undefined){
                 ...
                 if(this.schoolFilterId!=null){
                  ...
                  if(this.checkedListJournal.length>0){
                    // convert list to string and separte by values from List od DB
                    let idsJournal = this.checkedListJournal.map(item => item.id).join(',');
                    let valuesJournal = this.checkedListJournal.map(item => item.value).join(',');

                    this.pubSearchList.push({
                      columnName: "PublicationSourceDBName",
                      searchType: this.andorTypeJournal,
                      searchId: idsJournal,
                      searchValue: valuesJournal,
                      rangeFrom: null,
                      rangeTo: null,
                    });
                  }
                  ...
                  // }
                   ...
                   // for ranking
                   if (this.rankingValue != undefined) {
                    let rankingId = this.databaseDropDownRanking
                      .filter(item => this.rankingValue.includes(item.value))
                      .map(item => item.id)
                      .join(',');
                    ...
                    this.pubSearchList.push({
                      columnName: "Ranking",
                      searchType: "In",
                      searchId: null, // or rankingId, if needed
                      searchValue: this.rankingValue,
                      rangeFrom: null,
                      rangeTo: null,
                    });
                  }
                  ...
                  if(this.level!=undefined){
                    let levelId= this.levelList.filter(x=>x.value==this.level);
                    this.levelId=levelId[0].id;
                    this.pubSearchList.push({
                      columnName: "Level",
                      searchType: "Exact",
                      searchId: this.levelId,
                      searchValue: this.level,
                      rangeFrom: null,
                      rangeTo: null,
                    });
                  }
               ...
               getSelectedValueJournal(status: boolean, value: string, id: any): void {
                if (status) {
                  this.selectDBJournal.push(value);
                  this.checkedListJournal.push({ id: id, value: value });
                } else {
                  // If the checkbox is unchecked, remove from selectDB and checkedList
                  const indexInSelectDBJournal = this.selectDBJournal.indexOf(value);
                  if (indexInSelectDBJournal !== -1) {
                    this.selectDBJournal.splice(indexInSelectDBJournal, 1);
                  }
                  const indexInCheckedListJournal = this.checkedListJournal.findIndex(item => item.value === value);
                  if (indexInCheckedListJournal !== -1) {
                    this.checkedListJournal.splice(indexInCheckedListJournal, 1);
                  }
                }
                this.currentSelectedJournal = { checked: status, name: value };
                console.log(this.checkedListJournal);
