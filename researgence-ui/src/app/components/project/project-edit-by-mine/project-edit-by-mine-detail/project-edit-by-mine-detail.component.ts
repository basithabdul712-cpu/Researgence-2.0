...
@Component({
  selector: 'app-project-edit-by-mine-detail',
  templateUrl: './project-edit-by-mine-detail.component.html',
  styleUrls: ['./project-edit-by-mine-detail.component.scss','../../../../../assets/given/newcss/style.css']
})
export class ProjectEditByMineDetailComponent implements OnInit {
  ...
  constructor( private router:Router,private authService:AuthService,private scoreservice:ScorebookService,private modalService: NgbModal,
    ...
    fetchdata(){
      ...
      let storedTitle = localStorage.getItem('editProjectMineSearch');
          ...
          this.scoreservice.fetchProjectData(checkString).subscribe((x: HttpResponse<any>)=>{ 
            ...
            for(let i=0;i<this.dataList.length;i++){
              ...
              if(this.dataList[i].grantDate!=null){
                ...
                if (this.dataList[i].projectStageLifeCycle != null) {
                  const projectStageLifeCycle = this.dataList[i].projectStageLifeCycle.split(',');
        
                  this.dataList[i].projectStageLifeCycle = projectStageLifeCycle.map(item => {
                    const [name, value] = item.split(':');
                    return { name, value };
                  });
        
                  // Assigning variable for showing values dynamically in honeycomp
                  for (let t = 0; t < this.dataList[i].projectStageLifeCycle.length; t++) {
        
                    if (this.dataList[i].projectStageLifeCycle[t].name == "Filed") {
                      this.dataList[i].filed = this.dataList[i].projectStageLifeCycle[t].value;
                    }
                    if (this.dataList[i].projectStageLifeCycle[t].name == "Published") {
                      this.dataList[i].published = this.dataList[i].projectStageLifeCycle[t].value;
                    }
                    if (this.dataList[i].projectStageLifeCycle[t].name == "FER Issued") {
                      this.dataList[i].ferIssued = this.dataList[i].projectStageLifeCycle[t].value;
                    }
                    if (this.dataList[i].projectStageLifeCycle[t].name == "FER Replied") {
                      this.dataList[i].ferReply = this.dataList[i].projectStageLifeCycle[t].value;
                    }
                    if (this.dataList[i].projectStageLifeCycle[t].name == "Hearing Notice") {
                      this.dataList[i].hearNotice = this.dataList[i].projectStageLifeCycle[t].value;
                    }
                    if (this.dataList[i].projectStageLifeCycle[t].name == "Granted") {
                      this.dataList[i].granted = this.dataList[i].projectStageLifeCycle[t].value;
                    }
                    if (this.dataList[i].projectStageLifeCycle[t].name == "Examined") {
                      this.dataList[i].examined = this.dataList[i].projectStageLifeCycle[t].value;
                      }
                    }
                  }
                }    
    ...
    setExcel():Observable<any>{
     ...
     exportexcel(){
      ...
      str = str.replace(/\"projectId\":/g, "\"Project ID\":");
      str = str.replace(/\"applicationNumber\":/g, "\"Application Number\":");
      str = str.replace(/\"projectNumber\":/g, "\"Project Number\":");
      str = str.replace(/\"projectTitle\":/g, "\"Project Title\":");
      ...
      str = str.replace(/\"projectOfficeName\":/g, "\"Project Office Name\":");
      str = str.replace(/\"inventorAffiliation\":/g, "\"Inventor Affiliation\":");
      str = str.replace(/\"applicantAffiliation\":/g, "\"Applicant Affiliation\":");
      str = str.replace(/\"projectStageId\":/g, "\"Project Stage Id\":");
      str = str.replace(/\"projectStageName\":/g, "\"Project Stage Name\":");
      str = str.replace(/\"abstract\":/g, "\"Abstract\":");
      str = str.replace(/\"domain\":/g, "\"Domain\":");
      str = str.replace(/\"projectStageLifeCycle\":/g, "\"Project Stage Life Cycle\":");
      str = str.replace(/\"technologAreas\":/g, "\"Technology Area\":");
      this.excelList = JSON.parse(str);
      this.excelList.forEach((x) => {
        delete x.rowNumber;delete x.isInHouse;delete x.inventors;delete x.applicants;delete x.ferIssuedDate;delete x.ferReplyDate;delete x.hearingNoticeDate;delete x.grantDate;delete x.projectCountryId;
        delete x.correspondance;delete x.projectApplicantTypeId;delete x.projectApplicantTypeName;delete x.projectOfficeId;  
      });
    ...
    captureAsPDF(val:number){
      ...
      if(val==1){
        ...
        if(this.dataListPdf.length>0){
          ...
          var printContents = document.getElementById('printProject').innerHTML;
    ...
    popupWin.document.write(
      '<html><head>' +
        ...
        '<link rel="stylesheet" href="./project-mine.component.scss"/>' +
    ...
    toEditProject(projId){
      this.router.navigate(['/Project/admin/details/'+projId+"/"+this.univId]);
    }
