...
@Component({
  selector: 'app-project-admin-edit',
  templateUrl: './project-admin-edit.component.html',
  styleUrls: ['./project-admin-edit.component.scss','./../../../../assets/given/selected.css','./../../../../assets/given/newcss/style.css']
})
export class ProjectAdminEditComponent implements OnInit {
    ...
    private menuService:MenuService,private numtoWords:NumberToWordsService, private router:Router, private http: HttpClient, private facultyservice: FacultiesService,private excel:ExcelExportService) { }
        ...
        ngOnInit() {
          ...
          //  this.authservice.RoleSelection(this.user.UniversityId,this.user.UserId).subscribe(x=>{
           ...
           this.gservice.fetchProjectUserDetail(this.projectId,this.universityId).subscribe((x:any)=>{
            console.log(x);
            this.projectList=x;
      ...
      if(this.fileProj!=null){
          this.uploadFileProject().then(()=>{
            ...
            this.facultyservice.addNewProject(data,this.user.UserId,this.userName).subscribe(x=>{
              ...
              this.router.navigate(['/Project/added/list']);
             });      
           });
         }
