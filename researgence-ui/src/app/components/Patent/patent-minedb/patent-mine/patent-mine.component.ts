...
@Component({
  selector: 'app-patent-mine',
  templateUrl: './patent-mine.component.html',
  styleUrls: ['./patent-mine.component.scss','../../../../../assets/given/selected.css',  '../../../../../assets/given/newcss/style.css', '../../../../../assets/given/newcss/bootstrap.min.css']
})
export class PatentMineComponent implements OnInit {
  ...
  fetchdata(){
    let storedTitle = localStorage.getItem('mineSearchPat');
...
exportexcel(){
    ...
    str = str.replace(/\"ferIssuedDate\":/g, "\"FERIssuedDate\":");
    str = str.replace(/\"ferReplyDate\":/g, "\"FERReplyDate\":");
    str = str.replace(/\"hearingNoticeDate\":/g, "\"HearingNoticeDate\":");
    str = str.replace(/\"grantDate\":/g, "\"GrantDate\":");
    this.excelList = JSON.parse(str);
    this.excelList.forEach((x) => {
      delete x.rowNumber;delete x.isInHouse;delete x.inventors;delete x.applicants;delete x.patentCountryId;
