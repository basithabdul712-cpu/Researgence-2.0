...
import { DfsPatent } from './dfsPatent';
import {DfsSourceTitleData } from './dfsSourceTitleData';
import { moveItemInArray, CdkDragDrop } from '@angular/cdk/drag-drop';
import { FacultiesService } from '../../faculties/faculties.service';

@Component({
  selector: 'app-patent-feedersystem',
  templateUrl: './patent-feedersystem.component.html',
  styleUrls: ['./../../../../assets/given/newcss/dfs-style/style.css','./../../../../assets/given/newcss/dfs-style/bootstrap.min.css','./patent-feedersystem.component.scss']
})
export class PatentFeedersystemComponent implements OnInit {
  checkValues: any;
  doi: string;
  id: number;
  checkTitle: any;
 ...
 appNoList:any=[];
  ...
  submitAll() {
    ...
    if(this.dfsPublication.publishedDate!=null){
        const date1 = this.parseDateString(this.tempPubDt);
        const date2 = this.parseDateString(this.tempFilDate);
    ...
    for(let i=0; i< this.tempAppUsers.length; i++){
        this.tempAppUsers[i].aliasUserUniversityId=parseInt(this.universityparamId);
        this.tempAppUsers[i].isUserAddressSame=this.hideApplicant;
        ...
        this.tempAppUsers[i].patentApplicantSeqNoUI=0;
    ...
    if(this.rfsEnable){
       this.patentType="RFS";
       this.rfsQueueId= localStorage.getItem("patentDFS")    
    }  
    else{
      this.patentType="DFS";
      this.rfsQueueId=null;
    }

    if (inventors.length > 0) {
      this.facService.savePatentDFS(data, this.user.UserId, this.roleId, this.rfsQueueId,this.sameInvApp,this.patentType).subscribe(x => {
  ...
  prepareInventors() {
    return this.tempusers.map(user => {
      return {
        ...user,
        aliasUserUniversityId: parseInt(this.universityparamId),
  ...
  addNewDetails(val){
      ...
      newEdit(val,desc){
        ...
        const targetArray = desc === 'Inventors' ? this.feeders : this.applicantFeeders;
          ...
          if(targetArray[0].enableHome==true||targetArray[0].enableAppHome==true){
              if(desc === 'Inventors'){
                this.enableHome=true;
                this.enableAway=false;
              }
  ...
  SearchIPDataAway(text: string, val, author,desc) {
    ...
    parseDateString(dateStr) {
      // "08/07/2025 00:00:00" -> "2025-07-08T00:00:00"
    ...
    changeReqDt(val){
      ...
      console.log(val);
      this.dfsPublication.examinationDate=val;
    ...
    changetype(val){
      console.log(val);
       let filterCountry=this.patCountryList.filter(x=>x.id==val);
       this.dfsPublication.patentOfficeName=filterCountry[0].value;

    }

    changeDomain(val){
      let filterDomain = this.domainList.filter(x=>x.id==val);
      this.dfsPublication.patentDomainName=filterDomain[0].value;

    }

    changeAppStatus(val){
      let filterStatus = this.appStatusList.filter(x => x.applicationStatusId==val);
       this.dfsPublication.patentStatusName=filterStatus[0].applicationStatus;
    ...
    searchMatchAppNo(modal: any){

      this.facService.getExistingAppNo(this.searchAppNO).subscribe(x=>{
        this.appNoList=x;
        this.modalService.open(modal);
        if(this.appNoList.length>0){
            this.erromessage="Application Number already exist";
        }
        else{
         this.erromessage="Application Number not found";
        }
    ...
    proceedMatch(){
      ...
      this.dfsPublication.applicationNumber=this.searchAppNO;
      this.checkAppno=true;
      this.modalService.dismissAll();

    }

    close(){
      this.checkAppno=false;
      this.modalService.dismissAll();
    }
